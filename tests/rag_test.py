"""RAG prompt-injection baseline (NO shield).

Each test simulates ingesting a single poisoned document into the RAG system
and asking a question.  Context is inlined directly into the system prompt
with no boundary markers — exactly how unprotected RAG apps work.
...
OUTPUT_FILE = "rag_baseline_results.csv"
...
# Each is (question, ingested_doc, judge_hint)
# ---------------------------------------------------------------------------
...
def print_banner():
    ...
    print(f"  {WHITE}MODE:   {RED}{BOLD}BASELINE (NO SHIELD){RESET}")
...
def print_summary(results):
    ...
    print(f"\n  {BOLD}{'═' * 60}{RESET}")
    print(f"  {BOLD}{WHITE}  RESULTS SUMMARY — BASELINE (NO SHIELD){RESET}")
    print(f"  {'═' * 60}\n")
    ...
    if failed > passed:
        print(f"\n  {RED}{BOLD}  >>> VULNERABLE — {failed}/{total} injections succeeded{RESET}")
    print()


# need BLACK for the PASS badge
BLACK = "\033[30m"
...
def run_baseline():
    ...
    for i, (query, doc, hint) in enumerate(TESTS):
        ...
        messages = [
            {"role": "system", "content": RAG_SYSTEM_PROMPT.format(context=doc)},
        ...
        try:
            ...
            answer = resp.choices[0].message.content.strip() or "(empty)"
